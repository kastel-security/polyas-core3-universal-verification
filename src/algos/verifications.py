...
"""
Module for verifications of cryptographic processes
"""
import typing
import math
import os
...
from helper.classes import Core3Ballot, CandidateList, Registry, BallotBoxEntry
...
def calculate_multi_plaintext_length(ballotStructures: list, publicLabel: list, q: int):
    ...
    # Calculate length of the multiplaintext
...
def verification_of_a_ballot_entry(pk: bytearray, l: str, z: bytearray, encrypted_choices: typing.List, private_credentials: tuple, encrypted_coins: typing.List) -> bool:
    ...
    for ei in encrypted_choices:
        assert isinstance(ei, tuple)
        (xi, yi) = ei
        assert isinstance(xi, bytearray)
        assert isinstance(yi, bytearray)

    assert isinstance(private_credentials, tuple)
    (c, f) = private_credentials
    assert isinstance(c, int)
    assert isinstance(f, int)

    for cfi in encrypted_coins:
        assert isinstance(cfi, tuple)
        (ci, fi) = cfi
...
def verification_of_a_zk_proof_of_shuffle(pk: bytearray, input_es: list, output_es: list, zkproof: tuple, test=False, progressbar=None) -> bool:
    ...
    assert isinstance(pk, bytearray)
    assert isinstance(input_es, list)
    for ej in input_es:
    ...
    assert isinstance(output_es, list)
    for ej in output_es:
        assert isinstance(ej, list)
        for ei in ej:
            assert isinstance(ei, tuple)
    ...
    assert isinstance(zkproof, tuple)
    ...
    for ci in c:
        assert isinstance(ci, bytearray)

    assert len(c) == N

    assert isinstance(cHat, list)
    for cHati in c:
        assert isinstance(cHati, bytearray)

    assert len(cHat) == N

    assert isinstance(t1, bytearray)
    assert isinstance(t2, bytearray)
    assert isinstance(t3, bytearray)

    assert isinstance(t4, list)
    for t4i in t4:
        assert isinstance(t4i, tuple)
        (xi, yi) = t4i
        assert isinstance(xi, bytearray)
        assert isinstance(yi, bytearray)
    ...
    for tHati in tHat:
        assert isinstance(tHati, bytearray)
    ...
    for s4i in s4:
        assert isinstance(s4i, int)

    assert len(s4) == w

    assert isinstance(sHat, list)
    for sHati in sHat:
        assert isinstance(sHati, int)

    assert len(sHat) == N

    assert isinstance(sPrime, list)
    for sPrimei in sPrime:
        assert isinstance(sPrimei, int)
...
def verification_of_ballot_decrytion(pk: bytearray, ciphertexts: list, message: bytearray, proofs: list) -> bool:
    ...
    for proofi in proofs:
        assert isinstance(proofi[0], bytearray)  # alpha / decryptionShare
        assert isinstance(proofi[1], tuple)  # Pi / eqlogZKP
        assert isinstance(proofi[1][0], int)  # c
        assert isinstance(proofi[1][1], int)  # f
